...
void printHeap(int *heap, int size, int node=0, int d=0) {
    ...
    cout << string(3*d, ' ') << heap[node] << endl;
    if((2 * node + 1) <= (size - 1)) {
...
void remove(int* heap, int key, int & size) {
    // TODO: case where all elements are equal to key
    for(int i = 0; i < size; i++) {
        if(heap[i]==key) {
            heap[i] = heap[size-1];
            size -= 1;
...
int* mergeHeap(int* heap1, int* heap2, int size1, int size2) {
    ...
    int* heapNew = new int[size1 + size2];
    for(int i = 0; i < size1; i++) {
        heapNew[i] = heap1[i];
    }
    for(int i = 0; i < size2; i++) {
        heapNew[i+size1] = heap2[i];
    }
    heapify(heapNew, size1 + size2);
    return heapNew;
}
