...
bool delete_node(Node*& root, int key) {
  ...
  // case 3: target has two children
    ...
    Node* finder = target->left;
    while(finder->right != NULL) {
      finder = finder->right;
    }
    Node* finderParent = find_parent(root, finder);
    if(finderParent->right == finder){
      if(finder->left) {
        finderParent->right = finder->left;
      } else {
        finderParent->right = NULL;
      }
    } else {
      if(finder->left) {
        finderParent->left = finder->left;
      } else {
        finderParent->left = NULL;
      }
    }
    target->key = finder->key;
    delete finder;
    return true;
  }
  // free target
...
int depth( Node* root, Node* x ) {
  if(root == x) return 0;
  if(x->key < root->key) return 1 + depth(root->left, x);
  else return 1 + depth(root->right, x);
}
