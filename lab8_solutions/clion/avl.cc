...
int heightL( Node * x ) {
  if (x == NULL) return -1;
  int heightL;
  if (x->left) heightL = x->left->height;
  else heightL = -1;
  return heightL;
}
...
int heightR( Node * x ) {
  if (x == NULL) return -1;
  int heightR;
  if (x->right) heightR = x->right->height;
  else heightR = -1;
  return heightR;
}
  //
...
void balance( Node *& x ) {
  ...
  int balance = heightL(x) - heightR(x);
  if (balance > 1) { // Node is unbalanced (left higher)
    balance = heightL(x->left) - heightR(x->left);
    if (balance > 0) { // Case: LL
      rotateRight(x);
    } else if (balance < 0) { // Case: LR
      doubleRotateRight(x);
    }
  } else if (balance < -1) { // Node is unblanced (right higher)
    balance = heightL(x->right) - heightR(x->right);
    if (balance < 0) { // Case: RR
      rotateLeft(x);
    } else if (balance > 0) { // Case: RL
      doubleRotateLeft(x);
    }
