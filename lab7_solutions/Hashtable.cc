...
int Hashtable::hash2(int k, int hashChoice) {
  switch(hashChoice){
    case 1:
      return (_size - 1) - (k % (_size - 1)) + 1;
      break;
    case 2:
      return k % (_size - 2) + 1;
      break;
...
void Hashtable::qinsert(int k) {
    ...
    // ************* ADD YOUR CODE HERE *******************
    if(k == -1) {
      cout << "Cannot insert -1" << endl;
      return;
    }

    int hash = Hashtable::hash(k);

    for(int i = 1; i < ceil(_size/2); i++) {
      if(_table[hash] == -1) {
        _table[hash] = k;
        Hashtable::tallyProbes(i);
        return;
      } else {
        hash = (hash + (i*i)) % _size;
      }
...
void Hashtable::dinsert(int k, int hashChoice) {
	// Insert k in the hash table.
    ...
    int hash = Hashtable::hash(k);
    int hash2 = Hashtable::hash2(k, hashChoice);
